...
const CustomerDashboard = () => {
  ...
  const fetchOrderTimeline = async (orderId) => {
    try {
      ...
      console.warn('Timeline fetch failed, using demo data');
      setOrderTimeline([
        { event_type: 'order.placed', created_at: new Date().toISOString() },
        { event_type: 'restaurant.accepted', created_at: new Date(Date.now() + 300000).toISOString() }
      ]);
    }
  };
    ...
    <div className="min-h-screen">
      ...
      <main className="pl-20 md:pl-64 min-h-screen">
        ...
        <header className="sticky top-0 z-40 bg-background/60 backdrop-blur-xl border-b border-white/5 px-8 py-6 flex justify-between items-center">
            <div>
               <h2 className="text-xl font-bold">Good Evening, {user?.name?.split(' ')[0] || 'Customer'} 👋</h2>
          ...
          <AnimatePresence mode="wait">
            ...
            {view === 'orders' && (
              ...
              >
                  <div className="flex justify-between items-center mb-8">
                    ...
                    {selectedOrder && (
                      <button 
                        ...
                        className="px-4 py-2 glass-card text-xs font-bold uppercase tracking-widest"
                      >
                        ← Back to Orders
                      </button>
                    )}
                  ...
                  {selectedOrder ? (
                    <OrderDetails order={selectedOrder} timeline={orderTimeline} />
                  ) : (
               ...
               {cart.length > 0 && (
                 <div className="pt-8 border-t border-white/5 space-y-6">
                   <div className="space-y-4">
                     <label className="block text-sm font-bold text-white/60 uppercase tracking-widest">Delivery Address</label>
                     <textarea
                       value={deliveryAddress}
                       onChange={(e) => setDeliveryAddress(e.target.value)}
                       placeholder="Enter your complete delivery address..."
                       className="w-full bg-white/5 border border-white/10 rounded-xl p-4 focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500/50 outline-none transition-all text-sm resize-none"
                       rows={3}
                     />
                   </div>
  ...
  <div onClick={onClick} className="glass-card group overflow-hidden cursor-pointer h-full flex flex-col relative">
    ...
    <div className="p-6 flex-1 flex flex-col">
      ...
      <div className="mt-auto flex items-center gap-6 border-t border-white/5 pt-5">
        ...
        <div className="flex items-center gap-1.5 text-[10px] font-black text-white/40 uppercase tracking-widest">
           <MapPin size={12} className="text-emerald-500" /> {rest.address?.split(',')[0] || 'Nearby'}
        </div>
      </div>
    </div>
  </div>
...
const OrderDetails = ({ order, timeline }) => (
  <div className="space-y-8">
    <div className="glass-card p-8">
      <div className="flex justify-between items-start mb-8">
        <div>
          <h3 className="text-2xl font-black tracking-tight mb-2">Order #ORD-{order.id}</h3>
          <p className="text-white/40 text-sm">{new Date(order.created_at).toLocaleString()}</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          <h4 className="font-bold mb-4 text-white/60 uppercase tracking-widest text-sm">Delivery Details</h4>
          <div className="space-y-3">
            <div className="bg-white/5 p-4 rounded-xl">
              <p className="text-xs text-white/40 uppercase tracking-widest mb-1">Delivery Address</p>
            ...
            <div className="bg-white/5 p-4 rounded-xl">
              ...
              <p className="font-black text-emerald-400 text-xl">₹{order.total_amount}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="glass-card p-8">
      <h4 className="font-bold mb-6 text-white/60 uppercase tracking-widest text-sm">Order Timeline</h4>
      <div className="space-y-4">
        {timeline.map((event, i) => (
          <div key={i} className="flex items-center gap-4">
            <div className="w-3 h-3 rounded-full bg-emerald-500"></div>
            <div className="flex-1">
              <p className="font-bold text-sm">{event.event_type.replace('order.', '').replace('restaurant.', '').replace('delivery.', '').replace(/\./g, ' ').toUpperCase()}</p>
...
const StatusBadge = ({ status }) => {
    const colors = {
        ...
        'RESTAURANT_ACCEPTED': 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20 shadow-yellow-500/5',
        'READY_FOR_PICKUP': 'bg-teal-500/10 text-teal-400 border-teal-500/20 shadow-teal-500/5',
        'PARTNER_ASSIGNED': 'bg-orange-500/10 text-orange-400 border-orange-500/20 shadow-orange-500/5',
        'PICKED_UP': 'bg-purple-500/10 text-purple-400 border-purple-500/20 shadow-purple-500/5',
        'OUT_FOR_DELIVERY': 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20 shadow-indigo-500/5',
    ...
    return (
      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black border tracking-widest shadow-lg ${c} flex items-center gap-2`}>
        <div className={`w-1.5 h-1.5 rounded-full ${status === 'DELIVERED' ? 'bg-emerald-400' : 'bg-current pulse'}`} />
        {status.replace(/_/g, ' ')}
      </span>
    );
};
