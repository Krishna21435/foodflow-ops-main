...
async function connectRabbitMQ() {
  try {
    ...
    // Subscribe to status updates from other services
    const q = await channel.assertQueue('', { exclusive: true });
    channel.bindQueue(q.queue, 'order_events', 'restaurant.accepted');
    channel.bindQueue(q.queue, 'order_events', 'restaurant.ready');
    channel.bindQueue(q.queue, 'order_events', 'delivery.assigned');
    channel.bindQueue(q.queue, 'order_events', 'delivery.picked_up');
    channel.bindQueue(q.queue, 'order_events', 'delivery.out_for_delivery');
    channel.bindQueue(q.queue, 'order_events', 'delivery.delivered');
